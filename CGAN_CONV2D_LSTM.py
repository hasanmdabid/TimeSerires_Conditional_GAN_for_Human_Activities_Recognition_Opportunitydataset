...
def define_discriminator(in_shape=(32, 107, 1), n_classes=18):
    ...
    merge = Concatenate(name='Discriminator-Combine-Layer')(
        [in_image, li])  # 32x107x2 (2 channels, 1 for image and the 1 for labels)

    # down-sample: This part is same as unconditional GAN upto the output layer.
    # We will combine input label with input image and supply as inputs to the model.
    fe = Conv2D(filters=64, kernel_size=(3, 1), strides=(1, 1), padding='same', name='Discriminator-Hidden-Layer-1')(
        merge)  # 16x54x64
    # fe = BatchNormalization(momentum=0.5)(fe)
    fe = LeakyReLU(alpha=0.2, name='Discriminator-Hidden-Layer-Activation-1')(fe)
    fe = Dropout(0.2)(fe)
    # down-sample
    fe = Conv2D(filters=128, kernel_size=(3, 1), strides=(1, 1), padding='same', name='Discriminator-Hidden-Layer-2')(
        fe)  # 8x27x128
    # fe = BatchNormalization(momentum=0.5)(fe)
...
def define_generator(latent_dim, n_classes=18):
    ...
    gen = Reshape((8, 107, 32), name='Generator-Foundation-Layer-Reshape-1')(gen)  # Shape=8x8x32
    ...
    gen = Conv2DTranspose(filters=64, kernel_size=(3, 1), strides=(1, 1), padding='same',
                          name='Generator-Hidden-Layer-1')(merge)  # 16x107x64
    ...
    gen = Conv2DTranspose(filters=32, kernel_size=(3, 1), strides=(1, 1), padding='same',
                          name='Generator-Hidden-Layer-2')(gen)  # 32x107x64
    # gen = BatchNormalization(momentum=0.5)(gen)
    ...
    gen = LSTM(107, return_sequences=True, name='Generator-LSTM-Layer')(gen)
...
def saveResultsCSV(gen_activation, epochs, batch_per_epoch, d_loss_real, d_loss_fake, g_loss):
    ...
    fileString = './results/D_and_G_model_Loss_1k_CONV2D_LSTM.csv'
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs, n_batch):
    ...
    g_model.save('CGAN_100_epochs_64Batch_Withmajority_conv2d_lstmh5')
