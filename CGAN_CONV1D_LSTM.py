...
def define_discriminator(in_shape=(32, 107), n_classes=18):
    ...
    # downsample to 16x16
    fe = Conv1D(64, 3, strides=2, padding='same')(concat)
    ...
    # downsample one more
    fe = Conv1D(8, 3, strides=2, padding='same')(fe)
    fe = BatchNormalization()(fe)
    fe = LeakyReLU(alpha=0.2)(fe)
    fe = Dropout(0.2)(fe)
    ...
    output_layer = Dense(1, activation='sigmoid', name='Discriminator-Output-Layer')(fe)  # Output Layer
...
def define_generator(latent_dim, n_classes=18):
    ...
    gen = Conv1D(filters=54, kernel_size=3, padding='same', name='Generator-Hidden-Layer-1')(merge)  # 16x107x64
    ...
    gen = Conv1D(filters=108, kernel_size=3, padding='same', name='Generator-Hidden-Layer-2')(gen)  # 16x107x64
    #gen = BatchNormalization(momentum=0.5)(gen)
    gen = LeakyReLU(alpha=0.2, name='Generator-Hidden-Layer-Activation-2')(gen)
    gen = UpSampling1D(size=2)(gen)

   # output
    out_layer = LSTM(107, return_sequences=True, name='Generator-OUT-Layer')(gen)  # 32x107
    # define model
...
def saveResultsCSV(gen_activation, epochs, batch_per_epoch, d_loss_real, d_loss_fake, g_loss):
    ...
    fileString = './results/D_and_G_model_Loss_1K_64Batch_conv1d_lstm.csv'
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs, n_batch):
    ...
    g_model.save('CGAN_CONV1D_LSTM_OPPO_1K_epochs_64Batch_without_BatchNormalization.h5')
